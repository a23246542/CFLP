...
$.extend({
    ...
    /*lotteryList:function () {
        var url = lottery_URL + "/unify-activity/act/cf20180507/list/my";
        var param = {
            activityPeriods:"CF20180507",
            companyId:"8"
        };
        $.JsonAjax(url,param,function(data){
            if (data.code == 0){
                data = data.data;
                var list = "";
                for(var i in data){
                    list += "<li><span></span><span>"+data[i].giftTime +" "+ $.editAcount(data[i].accountNo)+" 抽中了"+data[i].giftName+"</span></li>";
                }
                $(".slider ul").empty().append(list);
                $(".lottery").slide({mainCell:".slider ul",autoPlay:true,effect:"topMarquee",vis:7,interTime:50,trigger:"click"});
            }
        });

    },
    //编辑账号
    editAcount:function(accountNo){
        return accountNo.substring(0,2)+"****"+accountNo.substring(accountNo.length-2,accountNo.length);
    },*/
});