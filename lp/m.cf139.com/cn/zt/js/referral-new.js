...
function refreshCaptcha(img){
    $(img).attr("src",id_url+"/cn/pc/img?r="+Math.random());
}
$(function() {
...
$('#phone').focus(phoneCheck);
    ...
    $('#pc_yanZ').focus(pc_yanZCheck);
    function pc_yanZCheck(){
        messageInfo.alert("pc_yanZ");
    }
    ...
    $.post( mis_url + "/public/cfd/customerRelation/getRelationUrlByPhone",{phone:phone},function (data) {
        ...
        if(data.code!=1){
            if(data.msg == "该手机号未开通CFD真实账户，不存在推荐链接"){
                $("#copied").val("");
                messageInfo.info( 'phone', "此手机号尚未注册真实账号，请点击此处<a class='item-b user user-m' href='"+id_url+"/cn/mobile/rcfd_account' >注册</a>");
            }
            return;
        }
        $(".phone-tip").html("");
