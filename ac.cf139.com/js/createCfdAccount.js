...
function setToHiddenFields(tracker) {//設定tracker
...
var checkIdNumberWhiteList = function(idNumber){ //檢查白名單??
    var res = false;
    $.ajax({
        cache: false,
        type: "POST",
...
$(function () {//密碼的時候可見 沒打得時候password不可見

    $('.try a').click(function(){
        $('#loginKuang').hide();//用在common-html 類似login.html的登祿視窗 應該是已經棄用
    });
...
//  验证推荐号码
    ...
    validatePhoneVNo:function (div,yanZ,phone) {
        ...
        if(!this.validatePhoneVNoByAjax(yanZ,phone)){//驗證手機驗證碼格式
            // 手机验证码不正确
     ...
     * @returns {boolean}
     */
    generatePhoneCaptcha : function (div,pram) { //看發送驗證碼有沒有成功 回傳結果
        var result = false;
        jQuery.ajax({
            type : "POST",
    ...
    validateUserNameLocal:function (div, name) {//說是驗證電話 但內容在驗證手機?
        ...
        if(!regPhone.test(name)){//格式不正确
            messageInfo.info(div,"真实姓名只能为中文或英文！");//??怪怪的
            return false;
        }
        messageInfo.success(div);
        return true;
    ...
    checkCustomerByPhone:function (phone,isDemo) {//電話驗證 ?
        var result;
        jQuery.ajax({
            type : "POST",
